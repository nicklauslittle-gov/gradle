...
class ReproducibleArchivesIntegrationTest extends AbstractIntegrationSpec {
    ...
    def "reproducible #taskName for directory with timestamps - #files"() {
        ...
        buildFile << """
            task ${taskName}(type: ${taskType}) {
                reproducibleFileOrder = true
                reproducibleFileTimestamp = java.time.Instant.ofEpochSecond(1248072216).toEpochMilli() + 1
                from 'src'
                destinationDirectory = buildDir
                archiveFileName = 'test.${fileExtension}'
                filePermissions {}
                dirPermissions {}
                doLast {
                    def file = file('build/test.${fileExtension}')
                    def sdf = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss")

                    if ('zip'.equals('${fileExtension}')) { // Open the zip file
                        new java.util.zip.ZipFile(file).withCloseable { zip ->
                            // Iterate through all entries (files and folders)
                            zip.entries().each { entry ->
                                // Get last modified time as a Date object
                                def lastModified = new java.util.Date(entry.time)
                                println "File: \${entry.name} | Last Modified: \${sdf.format(lastModified)}"
                            }
                        }
                    } else {
                        // Use tarTree to treat the tar as a file tree
                        tarTree(file).visit { FileVisitDetails fileDetails ->
                            // Print the path and last modified time
                            println "File: \${fileDetails.path}, Last Modified: \${new Date(fileDetails.lastModified)}"
                        }
                    }
                }
            }
